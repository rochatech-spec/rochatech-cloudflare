...
def guard_render_app():
    global a
    p,q=bounds('renderApp'); seg=a[p:q]
    if 'function renderApp(' in seg: seg=seg.replace('function renderApp(','function ritmoRenderAppCore(',1)
    elif 'async function renderApp(' in seg: seg=seg.replace('async function renderApp(','async function ritmoRenderAppCore(',1)
    else: raise SystemExit('Assinatura renderApp não encontrada')
    wrapper="\nfunction renderApp(...args){if(state.data&&!ritmoSessionUnlocked()){showLock('launch');return}return ritmoRenderAppCore(...args)}\n"
    a=a[:p]+seg+wrapper+a[q:]

# Revalidação de senha dentro de sessão válida: não cria nova sessão e não guarda senha.
needle="  const userId=s.user_id;\n  if(path==='/api/bootstrap'&&request.method==='GET')return json(await bootstrap(env,userId));"
if needle not in w: raise SystemExit('Ponto /api/bootstrap não encontrado')
...
replace_func('loadApp',r'''async function loadApp(afterLogin=false){try{const data=await api('/api/bootstrap');state.data=data;state.page='home';state.settingsSub=null;applyTheme(data.settings?.theme||localStorage.getItem('ritmo:theme')||'system');startSync();if(afterLogin)ritmoMarkUnlocked();if(await maybeLock(!afterLogin))return;renderApp();if(afterLogin)toast('Tudo certo. Bem-vindo ao Ritmo.')}catch(err){if(err.status===401)renderAuth();else renderAuth('Não foi possível carregar sua conta agora.')}}
''')

replace_func('logout',r'''async function logout(){clearTimeout(ritmoLockTimer);const k=ritmoUnlockKey(),ak=ritmoActivityKey();try{if(k)sessionStorage.removeItem(k);if(ak)sessionStorage.removeItem(ak)}catch{}try{await api('/api/auth/logout',{method:'POST',body:'{}'})}catch{}state.data=null;state.authMode='login';renderAuth()}
''')

# Impede qualquer renderização privada antes do desbloqueio, mesmo se o boot mudar.
guard_render_app()

a=a.replace('<strong>Bloqueio automático</strong><small>Solicitar desbloqueio após inatividade</small>','<strong>Bloqueio automático</strong><small>Em segundo plano, bloqueia somente após o tempo escolhido</small>',1)
a=a.replace('<strong>Sessão protegida</strong><small>Vinculada ao ID interno da conta, sem depender do nome de usuário.</small>','<strong>Abertura protegida</strong><small>Ao reabrir o Ritmo após encerrar a sessão do app, confirme sua identidade.</small>',1)
a += "\n['pointerdown','keydown','touchstart'].forEach(ev=>document.addEventListener(ev,ritmoTouchActivity,{passive:true}));document.addEventListener('visibilitychange',()=>{if(document.hidden)clearTimeout(ritmoLockTimer);else maybeLock(false)});\n"
app.write_text(a)

css=cssp.read_text()+r'''
/* Ritmo V1 — tela de bloqueio isolada */
.secure-lock{position:fixed;inset:0;z-index:99999;min-height:100dvh;display:grid;place-items:center;padding:calc(24px + env(safe-area-inset-top)) 16px calc(24px + env(safe-area-inset-bottom));background:var(--bg,#F7F5EF);overflow:auto}
.secure-lock-card{width:min(100%,390px);display:flex;flex-direction:column;gap:12px;padding:24px;border-radius:28px;background:var(--panel,#fff);border:1px solid var(--line,rgba(15,76,92,.09));box-shadow:0 18px 55px rgba(25,40,45,.09)}
.secure-lock-card>.brand{align-self:center}.secure-lock-card>.eyebrow,.secure-lock-card>h2,.secure-lock-card>p{text-align:center}.secure-lock-card>h2{margin:0;font-size:24px}.secure-lock-card>p{margin:0;color:var(--muted);font-size:12px;line-height:1.5}
.secure-lock-avatar{display:flex;justify-content:center}.secure-lock-avatar-img{width:70px!important;height:70px!important;min-width:70px!important;max-width:70px!important;border-radius:50%!important;object-fit:cover;display:grid;place-items:center;background:color-mix(in srgb,var(--teal,#0F4C5C) 10%,var(--panel,#fff));font-size:24px;font-weight:700;color:var(--teal,#0F4C5C)}
.secure-account{text-align:center;display:flex;flex-direction:column;gap:2px}.secure-account small{color:var(--muted)}.secure-bio-btn,.lock-password-form .btn{width:100%;min-height:45px}.lock-divider{display:flex;align-items:center;gap:10px;color:var(--muted);font-size:10px}.lock-divider:before,.lock-divider:after{content:'';height:1px;background:var(--line);flex:1}.lock-password-form{display:grid;gap:10px}.secure-lock-note{font-size:9.5px;line-height:1.5;color:var(--muted);text-align:center}
.remember-account{display:flex;align-items:flex-start;gap:10px;padding:10px 11px;border-radius:14px;background:color-mix(in srgb,var(--teal,#0F4C5C) 5%,transparent)}.remember-account input{width:17px;height:17px;margin-top:2px;accent-color:var(--teal,#0F4C5C)}.remember-account span{display:flex;flex-direction:column;gap:2px}.remember-account strong{font-size:11px}.remember-account small{font-size:9.5px;color:var(--muted);line-height:1.4}
@media(max-width:520px){.secure-lock-card{padding:21px 18px;border-radius:25px;box-shadow:none}}
'''
cssp.write_text(css)
print('Ritmo V1: bloqueio robusto aplicado sem depender do boot; segundo plano respeita tempo e tela privada nunca renderiza antes do desbloqueio.')
