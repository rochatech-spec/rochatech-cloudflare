import { ApiError, fetchBootstrap, mutate, persistScope } from '../api/client'
import type { BootstrapData, FinancialScope, PendingMutation } from '../domain/types'
import { bumpMutationAttempt, clearScopeSnapshots, deletePendingMutation, enqueueMutation, getScopeSnapshot, listPendingMutations, saveScopeSnapshot } from '../offline/db'
...
export async function refreshScope(scope: FinancialScope) { return (await loadScope(scope, { forceNetwork: true })).data }
export async function invalidateFinancialCache() { await clearScopeSnapshots(); inflight.clear() }
...
export async function flushPendingMutations(): Promise<number> {
  ...
  try {
    ...
    for (const item of items) {
      try { await mutate(item.path, item.method, item.body); await deletePendingMutation(item.id); completed += 1 }
      catch (error) {
    ...
    if (completed) { await clearScopeSnapshots(); window.dispatchEvent(new CustomEvent('ritmo:synced', { detail: { completed } })) }
