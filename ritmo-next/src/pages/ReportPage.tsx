...
export function ReportPage({ initialScope, sharedAvailable, profileName }: { initialScope: FinancialScope; sharedAvailable: boolean; profileName: string }) {
  ...
  return <div className="page-stack">
    <header className="page-header"><div><small>RELATÓRIO</small><h1>Seu período, bem explicado</h1><p>Pessoal e Casal ficam separados. Transferência entre carteiras não vira receita nova.</p></div><button className="primary-button" type="button" onClick={pdf} disabled={!report || busy}><Icon name="report"/> Gerar PDF</button></header>
    <section className="premium-card report-filter"><div className="segmented report-scope"><button className={scope==='personal'?'active':''} onClick={()=>setScope('personal')} type="button">Pessoal</button>{sharedAvailable&&<button className={scope==='shared'?'active':''} onClick={()=>setScope('shared')} type="button">Casal</button>}</div><label><span>Data inicial</span><input type="date" value={from} onChange={(e)=>setFrom(e.target.value)}/></label><label><span>Data final</span><input type="date" value={to} onChange={(e)=>setTo(e.target.value)}/></label><button className="secondary-button" type="button" onClick={load} disabled={busy}>{busy?'Aplicando…':'Aplicar'}</button></section>
