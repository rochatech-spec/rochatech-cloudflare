...
export function MovementsPage({ data, onNew, onEdit, onDelete }: { data: BootstrapData; onNew: (kind: 'income' | 'expense') => void; onEdit: (item: Movement) => void; onDelete: (item: Movement) => void }) {
  ...
  const totalIn = data.incomes.filter((item)=>item.date<=cutoff).reduce((sum, item) => sum + item.amount, 0)
  const totalOut = data.expenses.filter((item) => item.status === 'pago'&&item.date<=cutoff).reduce((sum, item) => sum + item.amount, 0)
  ...
  return <div className="page-stack">
    ...
    <div className="summary-strip"><span><small>Entradas realizadas</small><strong className="positive">{money(totalIn)}</strong></span><span><small>Saídas pagas</small><strong className="negative">{money(totalOut)}</strong></span><span><small>Resultado realizado</small><strong>{money(totalIn-totalOut)}</strong></span></div>
