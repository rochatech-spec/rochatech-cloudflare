...
export const api = {
  ...
  payDebt: (id: string, value: number, date?: string) => request<{payment:DebtPayment;debt:Debt}>(`/debts/${encodeURIComponent(id)}/payments`, { method: 'POST', body: JSON.stringify({ value, date }) }),
  ...
  addGoalValue: (id: string, value: number) => request<{contribution:GoalContribution;goal:Goal}>(`/goals/${encodeURIComponent(id)}/contributions`, { method: 'POST', body: JSON.stringify({ value }) }),
