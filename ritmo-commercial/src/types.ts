...
export type DebtPayment = { id: string; debtId: string; value: number; date: string; createdAt: string };
export type Goal = { id: string; name: string; target: number; saved: number; due?: string };
export type GoalContribution = { id: string; goalId: string; value: number; createdAt: string };
...
export type Bootstrap = { profile: Profile; transactions: Transaction[]; debts: Debt[]; debtPayments: DebtPayment[]; goals: Goal[]; goalContributions: GoalContribution[]; events: EventItem[]; files: StoredFile[] };
