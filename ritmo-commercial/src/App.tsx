...
import { canInstallPWA, enablePushNotifications, installPWA, isBiometricsAvailable, isNativeApp, loginWithBiometrics, registerBiometrics, shouldGateNativeSession, unlockNativeSession } from './services/nativeDevice';
...
export default function App() {
  ...
  useEffect(()=>{
    ...
    const name=profile.displayName||'Conta',ini=initials(name);setText('accountAvatar',ini);setText('profileAvatar',ini);setText('accountName',name);setText('profileName',name);setText('profileUser',name?'@'+name.toLowerCase().replace(/\s+/g,'.'):'@usuario');
    setText('profileIncome',money(totals.inc));setText('profileExpense',money(totals.exp));setText('profileBalance',money(totals.bal));setText('profileScore',transactions.length?`${Math.round(totals.rate)}%`:'—');document.getElementById('profileDonut')?.style.setProperty('--score',String(Math.round(totals.rate)));setText('profileStorage','Cloudflare D1 + R2');setText('profileNotifications',(profile.dueNotifications||profile.goalNotifications)?'Ativas':'Desativadas');
  ...
  useEffect(()=>{
    ...
    const setOpen=(open:boolean)=>{menu.classList.toggle('show',open);fab.classList.toggle('fab-open',open);fab.setAttribute('aria-expanded',String(open));menu.setAttribute('aria-hidden',String(!open));if(open)requestAnimationFrame(posMenu)};
    ...
    return()=>{fab.removeEventListener('pointerdown',down);fab.removeEventListener('pointermove',move);fab.removeEventListener('pointerup',end);fab.removeEventListener('pointercancel',end);fab.removeEventListener('click',click);document.removeEventListener('click',outside);window.removeEventListener('resize',resize)};
  },[authenticated]);

  async function busy<T>(fn:()=>Promise<T>,success?:string){if(loading)throw new Error('busy');setLoading(true);try{const r=await fn();if(success)notify(success);return r;}catch(e:any){if(e?.message!=='busy')notify(e instanceof ApiError?e.message:(e?.message||'Não foi possível concluir a operação.'));throw e;}finally{setLoading(false);}}
  ...
  function renderModal(){
    ...
    if(modal.kind==='privacy')return shell('Privacidade e dispositivo',<div className="list"><div className="row"><div className="row-main"><strong>Sincronização protegida</strong><small>Dados financeiros ficam no D1; arquivos no R2; biometria permanece no autenticador do aparelho.</small></div></div><div className="row"><div className="row-main"><strong>Notificações push</strong><small>Ative alertas nativos para vencimentos e metas.</small></div><button className="premium-btn btn-glass" id="enablePush" style={{height:36,padding:'0 11px'}}>Ativar</button></div><div className="row"><div className="row-main"><strong>Instalar PWA</strong><small>Instale o Ritmo como aplicativo no aparelho.</small></div><button className="premium-btn btn-glass" id="installPwa" style={{height:36,padding:'0 11px'}} disabled={!canInstallPWA()}>Instalar</button></div></div>);
