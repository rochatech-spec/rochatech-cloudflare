...
const sessionSecret=(env:any)=>{const s=env?.SESSION_SECRET||env?.JWT_SECRET;if(typeof s!=='string'||s.length<32)throw new Error('SESSION_SECRET_MISSING');return s}
...
export function hasSessionSecret(env:any){const s=env?.SESSION_SECRET||env?.JWT_SECRET;return typeof s==='string'&&s.length>=32}
